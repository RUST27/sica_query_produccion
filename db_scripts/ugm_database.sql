...
create function public.fn_actualizar_id_reverso(p_id_transaccion bigint, p_id_reverso bigint) returns void
    language plpgsql
as
$$
BEGIN
    -- Actualizar el campo id_reverso con el valor proporcionado
    UPDATE transacciones_ugmex
    SET id_reverso = p_id_reverso
    WHERE id_transaccion = p_id_transaccion;

    -- Confirmar si se actualizó algún registro
    IF NOT FOUND THEN
        RAISE WARNING 'No se encontró el registro con id_transaccion: %', p_id_transaccion;
    END IF;
END;
$$;

alter function public.fn_actualizar_id_reverso(bigint, bigint) owner to postgres;
