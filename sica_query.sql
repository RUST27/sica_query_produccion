...
(
    id_gerencia        integer generated by default as identity
        primary key,
...
(
    id_grupo_usuario   integer generated by default as identity
        primary key,
...
(
    id_departamento    integer generated by default as identity
        primary key,
    descripcion        varchar(100),
    baja               boolean,
    id_gerencia        integer
...
(
    id_grupo_usuario_responsable integer generated by default as identity
        primary key,
    nombre                       varchar(100),
    descripcion                  text,
...
(
    id_clasificacion_actividad integer generated by default as identity
        primary key,
    nombre                     varchar(100),
    descripcion                text,
    baja                       boolean,
...
(
    id_grupo_actividad         integer generated by default as identity
        primary key,
    nombre                     varchar(100),
    descripcion                text,
    baja                       boolean,
...
(
    id_estatus         integer generated by default as identity
        primary key,
    descripcion        varchar(100),
    clave              varchar(10),
    fecha_creacion     timestamp default CURRENT_TIMESTAMP,
...
(
    id_usuario_sica    integer generated by default as identity
        primary key,
    responsable        boolean,
    jefe_area          boolean,
    correo             varchar(150),
...
(
    id_atencion                  integer   generated by default as identity not null,
...
CREATE OR REPLACE FUNCTION insertar_menus_y_submenus()
RETURNS void AS
$$
DECLARE
    id_menu_sica integer;
    id_menu_atenciones integer;
    id_menu_catalogos_sica integer;
BEGIN
    -- Insertar el menú padre 'SICA' si no existe y obtener su ID
    SELECT id_menu INTO id_menu_sica
    FROM tb_catalogo_menus 
    WHERE descripcion = 'SICA' AND id_sistema = 1;

    IF id_menu_sica IS NULL THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('SICA', 0 , 1)
        RETURNING id_menu INTO id_menu_sica;
    END IF;

    -- Insertar el menú padre 'Atenciones' si no existe y obtener su ID
    SELECT id_menu INTO id_menu_atenciones
    FROM tb_catalogo_menus 
    WHERE descripcion = 'Atenciones' AND menu_padre = id_menu_sica AND id_sistema = 1;

    IF id_menu_atenciones IS NULL THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Atenciones', id_menu_sica, 1)
        RETURNING id_menu INTO id_menu_atenciones;
    END IF;

    -- Insertar los submenús de 'Atenciones'
    IF NOT EXISTS (SELECT 1 FROM tb_catalogo_menus WHERE descripcion = 'Gestión Atenciones' AND menu_padre = id_menu_atenciones) THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Gestión Atenciones', id_menu_atenciones, 1);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM tb_catalogo_menus WHERE descripcion = 'Mis Atenciones' AND menu_padre = id_menu_atenciones) THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Mis Atenciones', id_menu_atenciones, 1);
    END IF;

    -- Insertar el menú padre 'Catálogos SICA' si no existe y obtener su ID
    SELECT id_menu INTO id_menu_catalogos_sica
    FROM tb_catalogo_menus 
    WHERE descripcion = 'Catálogos SICA' AND menu_padre = id_menu_sica AND id_sistema = 1;

    IF id_menu_catalogos_sica IS NULL THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Catálogos SICA', id_menu_sica, 1)
        RETURNING id_menu INTO id_menu_catalogos_sica;
    END IF;

    -- Insertar los submenús de 'Catálogos SICA'
    IF NOT EXISTS (SELECT 1 FROM tb_catalogo_menus WHERE descripcion = 'Departamento SICA' AND menu_padre = id_menu_catalogos_sica) THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Departamentos SICA', id_menu_catalogos_sica, 1);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM tb_catalogo_menus WHERE descripcion = 'Actividades SICA' AND menu_padre = id_menu_catalogos_sica) THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Actividades SICA', id_menu_catalogos_sica, 1);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM tb_catalogo_menus WHERE descripcion = 'Clasificación de Actividades SICA' AND menu_padre = id_menu_catalogos_sica) THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Clasificación Actividades SICA', id_menu_catalogos_sica, 1);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM tb_catalogo_menus WHERE descripcion = 'Grupo de Actividades SICA' AND menu_padre = id_menu_catalogos_sica) THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Grupos de Actividades SICA', id_menu_catalogos_sica, 1);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM tb_catalogo_menus WHERE descripcion = 'Usuarios SICA' AND menu_padre = id_menu_catalogos_sica) THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Usuarios SICA', id_menu_catalogos_sica, 1);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM tb_catalogo_menus WHERE descripcion = 'Grupos Responsables SICA' AND menu_padre = id_menu_catalogos_sica) THEN
        INSERT INTO tb_catalogo_menus (descripcion, menu_padre, id_sistema)
        VALUES ('Grupos Responsables SICA', id_menu_catalogos_sica, 1);
    END IF;
...
SELECT insertar_menus_y_submenus();
...
CREATE FUNCTION fn_sica_obtener_todos_usuarios_catalogo_consulta()
...
ALTER FUNCTION fn_sica_obtener_todos_usuarios_catalogo_consulta() OWNER TO postgres;
...
BEGIN
    ...
    -- Llamar a la función de consolidación para oficinas
    PERFORM public.fn_consolidador_registrarmovimiento_para_oficinas(
        _json,
        'u',
        'tb_sica_catalogo_actividades',
...
CREATE OR REPLACE FUNCTION public.fn_sica_grupo_resp_asignar_usuario_insertar(p_id_usuario integer, p_id_grupo_usuario_responsable integer) 
RETURNS void
LANGUAGE plpgsql
AS
$$
...
BEGIN
    ...
    JOIN public.tb_sica_catalogo_departamentos d ON ca.id_departamento = d.id_departamento
    WHERE ca.id_departamento = p_id_departamento;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.fn_sica_actividad_insertar(p_nombre character varying, p_descripcion text, p_tiempo numeric, p_baja boolean, p_id_grupo_actividad integer, p_id_grupo_usuario_responsable integer, p_usuario_creacion integer)
RETURNS integer
LANGUAGE plpgsql
AS
$$
...
BEGIN
    ...
    IF EXISTS (
        ...
        -- Actualizar el estatus de la atención a '2' (Leído)
        UPDATE public.tb_sica_atenciones
        SET id_estatus = 2,
            fecha_modificacion = CURRENT_TIMESTAMP
        WHERE id_atencion = p_id_atencion
